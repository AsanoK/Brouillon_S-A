...
\section{L'aventure}
\subsection{Fatigue}
%TODO : tableaux et listes
\subsubsection{Etats de fatigue - jauge de fatigue}
Il y  a 7 états de fatigue, avec des effets différents :
reposé : le personnage frais et dispo, pas d’effet particulier.
échauffé : l’effort a commencé, mais pour le moment, sans aucun problème.
essoufflé : les premières difficultés commencent à apparaître. Malus de -1 à tous les tests.
fatigué : la fatigue commence à se faire sentir, et à diminuer l’efficacité du personnage : -2 à tous les tests.
exténué : le personnage est amené dans ses derniers retranchements. Malus de -4 à tous les tests.
terrassé : le personnage est au-delà de ses limites, et le paiera très cher le lendemain. Malus de -6 à tous les tests, et commencera le lendemain avec un niveau de fatigue plus important : sans autre problème, le personnage passera la prochaine journée à l’état échauffé.
coma : Le personnage tombe dans l’inconscience après ses efforts trop importants.
...
Es correspond au seuil pour être essoufflé, F à fatigué, Ex à exténué, T à terrassé et C pour coma.
Calcul :
seuil
valeur
Ec
END/2 (arrondi au supérieur)
Es
END
F
END+3
...
Suivant la difficulté des efforts, un personnage gagne un point de fatigue après une certaine durée. Il est également possible de réduire la difficulté d’un effort en réussissant un test d’END+compétence, avec la compétence adaptée à l’effort. La difficulté de ce test est indiqué dans la table suivante. Un effort faible réduit n’est pas annulé, mais a une durée de 1h pour gagner un point de fatigue.
...
5 secondes
6
Un effort faible est par exemple de la marche à pied relativement dynamique.
Un effort normal est une course tranquille par exemple.
Un effort important est un sprint par exemple.
...
Pour faire appel à cette aide, un personnage doit réaliser un test de service+SOC, dont la difficulté est déterminée par la table suivante:

faveur
difficulté
mineure : petit prêt, aide sans risque, ...
2
moyenne
3
importante
...
5
\subsection{Renommée}
La renommée d’un personnage est une indication de la façon dont il est connu au sein d’une communauté. Il est utilisé pour déterminer l’attitude globale des personnages envers celui-ci.

score
niveau social équivalent
0
vagabond, prostituée, ...
1
voyageur, habitant pauvre
2
habitant moyen, chevalier errant
3
respectable citoyen
4
notable, bourgeois, bas clergé
5
basse noblesse locale
6
noblesse moyenne, haut clergé

Ce score peut être utilisé dans certains cas (en test simple), pour : 
limiter un problème avec les autorités
rechercher quelqu’un en particulier
obtenir une audience avec quelqu’un
être reconnu par quelqu’un
...
Le corps à corps correspond à la distance de combat à mains nues, soit environ de 0 à 1m
La mêlée est la distance normale de frappe pour les armes à une main : entre 1 et 2m.
La distance moyenne est dans un duel la distance à laquelle les deux adversaires se placent pour se jauger. C’est aussi la zone de frappe de bon nombre d’armes à deux main : entre 2 et 3m.
Enfin, la distance longue est presque exclusivement une distance d’observation, quoique certaines armes particulièrement longues puissent y être à l’aise : de 3 à 5 m.
...
l’adversaire doit lâcher son arme.
L’adversaire se retrouve déplacé d’un mètre dans une direction choisie par l’attaquant.
L’adversaire se retrouve jeté au sol.
L’adversaire ne peut plus agir avec un membre choisit tant qu’il n’a pas réussi à se débarrasser de la lutte (test opposé de Masse+Corps à corps gagné).
...
La table suivante liste une partie des modificateurs classiques sur les jets de combat en mêlée:
Raison
Modificateur
attaque de dos
-5 au test de défense
attaque de flanc
-2 au test de défense
attaque ciblée
-1 à l’attaque
ignorer l’armure
-3 à l’attaque
combat au bouclier
+1 à +5 au test de défense
combat à deux armes
+1 au test de défense, +2 au test de contre-attaque, nécessite une arme courte en main secondaire
...
TODO
En cas de réussite, le défenseur effectue un test de VIT+défense, avec un seuil dépendant du type de tir effectué:

Attaque
seuil de défense
arme de jet (couteaux, haches, javelots)
3
arme de trait (flèche, carreau)
4
...
Avant de déterminer, quels dégâts sont infligés, il faut savoir quelle zone de la cible est touchée. Pour un humanoïde, on lance 1d10 sur la table suivante:
...
Une attaque réussie inflige un nombre de dégâts égal à la VD de l’arme+marge du test d’attaque au défenseur+Bonus de dégâts(pour une attaque au corps à corps) . On soustrait à cette valeur l’armure de la cible avant de comparer cela à ses seuils de blessure. La valeur d’armure utilisée (tranchante, perforante ou contondante) dépend du type d’attaque utilisée.
